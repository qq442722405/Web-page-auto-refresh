...
from PySide6.QtWidgets import (
    ...
    QComboBox, QTextEdit, QAbstractSpinBox, QScrollArea
)
from PySide6.QtGui import QIcon, QPixmap, QPainter, QPen, QColor, QTextCursor, QGuiApplication, QImage, QShortcut, QKeySequence
...
class PersistentROIOverlay(QWidget):
    ...
    def mousePressEvent(self,event):
        ...
        if self.is_picking_point:
            ...
            event.accept()  # Keep swallowing input until release; never leak a click.
            return
    ...
    def mouseReleaseEvent(self,event):
        if event.button() == Qt.LeftButton and self.is_picking_point:
...
from PySide6.QtWebEngineCore import (
    QWebEnginePage, QWebEngineCertificateError, QWebEngineScript, QWebEngineProfile, QWebEngineSettings
)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # OCR 只创建一个 ddddocr 引擎。V17 原版这里和 OCRWorker 各创建了一份模型，
        # 会明显增加内存占用；现在唯一的引擎由 OCRWorker 持有。
        self.ocr = None
        self.ocr_busy = False
        self.ocr_thread = QThread(self)
        self.ocr_worker = OCRWorker()
        self.ocr = self.ocr_worker.ocr
        ...
        g_url.addWidget(self.test_operation_btn)
        self.operation_result_label = QLabel("就绪；后台点击不会移动系统鼠标")
        self.operation_result_label.setWordWrap(True)
        g_url.addWidget(self.operation_result_label)
        ...
        try:
            ...
            _write_runtime_log("WebEngine：Chromium/QWebEngine 内核，缓存=DiskHttpCache，Cookie持久化")
        except Exception as e:
        ...
        # 高性能 Chromium：恢复 GPU/硬件合成能力，同时保留全屏与媒体策略。
        try:
        ...
        # 每分钟记录一次进程状态，便于定位“运行一段时间后闪退/内存持续增长”。
    ...
    def log(self, text):
        try:
            ...
            text = str(text)
            _write_runtime_log(text.replace("\n", " | "))
    ...
    def on_load_finished(self, ok):
        ...
        if self.auto_operation_cb.isChecked():
            self.start_auto_timer()  # New countdown, never resume pre-navigation work.
        if ok:
            self.log("✅ 页面加载完毕")
    ...
    def write_diagnostic_log(self):
        try:
            ...
            url = self.webview.url().toString() if hasattr(self, "webview") else ""
    ...
    def start_point_picker(self):
        ...
        self.operation_feedback("请在网页上拾取；Esc 取消，不会点击网页")
    ...
    def perform_scheduled_operation(self):
        if self.operation_gate.pending is not None or self.picker_action is not None:
